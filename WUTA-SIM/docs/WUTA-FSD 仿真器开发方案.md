...
- Skidpad：两个圆（中心线半径 9\.125m），圆心距 18\.25m；外圈半径 10\.625m。
  入口/出口直道边界固定为 `x=±1.500m`，外圈端点按
  `y=±sqrt(10.625²-(9.125-1.500)²)=±7.399324m` 严格对齐，13 个外圈
  锥桶在两端点之间的外侧长弧等分（角间隔 22.643426°）。运行时以
  `perception_simulation/tracks/skidpad.yaml` 为准。
